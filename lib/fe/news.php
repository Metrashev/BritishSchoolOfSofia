...
class CFENewsListPage {
	...
	function getBodyHTML() {
		$bo =new CBONews();
		$data = $bo->getPagedList("/?cid=".$this->node['id'], $this->itemsPerPage, 'id,title,subtitle,picture,due_date,href,cid', "cid=".(int)$this->node['id']." AND lang='".LNG_CURRENT."'" , "due_date DESC");
		$data['node'] = $this->node;
