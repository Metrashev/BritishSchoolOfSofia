...
if($__mvc) {
	...
	$filter_session_name="filter_classes";
}
else  {
	...
	if(isset($in_edit_id)&&(int)$in_edit_id) {
		...
		$filter_session_name="filter_classes";
		$search=getclassesControls('search');
	}
}


$__del_var="hdDeleteclasses";
$__editTable="classes";
$fn_Delete="";
$fn_Delete="del_classes";

function del_classes($del_id) {
	$db=getdb();
	...
	$db->execute("delete from `classes` where id='{$del_id}'");
}
