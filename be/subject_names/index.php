...
if($__mvc) {
	...
	$filter_session_name="filter_subject_names";
}
else  {
	...
	if(isset($in_edit_id)&&(int)$in_edit_id) {
		...
		$filter_session_name="filter_subject_names";
		$search=getsubject_namesControls('search');
	}
}


$__del_var="hdDeletesubject_names";
$__editTable="subject_names";
$fn_Delete="";/*
$fn_Delete="del_subject_names";

function del_subject_names($del_id) {
	$db=getdb();
		
	$db->execute("delete from `subject_names` where id='{$del_id}'");
}*/
